...
if (args[0] === 'ghosttrace') {
    const ghosttrace = require('../ghosttrace/index.js');
    if (args[1] === 'run') {
        ghosttrace.runScenario();
    } else if (args[1] === 'export' && args[2]) {
        const zipPath = ghosttrace.exportBundle(args[2]);
        console.log(`Exported bundle to: ${zipPath}`);
    } else {
        console.log("Usage: xagent ghosttrace [run | export <run_id>]");
    }
    process.exit(0);
}
...
async function chat(promptText) {
    ...
    try {
        ...
        trace.addStep('context.loaded', 'context', 'ok', 'MEM_OK', Date.now() - startMem);
        if (mems && mems.length > 0) {
            finalPrompt += "\n\n[MEMORY]:\n" + mems.map(m => `- ${m.text}`).join('\n');
        }
    } catch (e) {
        trace.addStep('context.loaded', 'context', 'error', 'MEM_ERR', Date.now() - startMem, e.message);
    }
    stopSpinner();
    ...
    while (!finished) {
        ...
        try {
            ...
            if (!res.ok) {
                trace.addStep('inference.generate', 'inference', 'error', 'API_HTTP_ERR', Date.now() - startGen, `Status: ${res.status}`);
            ...
            if (msg.tool_calls && msg.tool_calls.length > 0) {
                ...
                if (loopCount >= MAX_LOOPS) {
                    ...
                    trace.addStep('tools.execute', 'policy', 'error', 'LOOP_PREVENTED', 0);
                    finished = true;
                    break;
                }
            ...
            stopSpinner();
            trace.addStep('inference.generate', 'inference', 'error', 'NET_ERR', Date.now() - startGen, e.message);
    ...
    // Automatically generate explanation in background
    if (trace._failed) {
        const explanation = compileExplanation(trace);
        generateReport(trace, explanation, promptText, finalContent);
    }
}
